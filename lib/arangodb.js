...
ArangoDB.prototype.define = function(modelDefinition) {
    ...
    if(modelDefinition.properties._key) throw new Error(g.f('{{ArangoDB}} property _key is reserved, use key instead'));
    if(modelDefinition.properties._id) throw new Error(g.f('{{ArangoDB}} property _id is reserved, use id instead'));
    if(modelDefinition.properties._rev) throw new Error(g.f('{{ArangoDB}} property _rev is reserved, use rev instead'));
    if(modelDefinition.properties._from) throw new Error(g.f('{{ArangoDB}} property _from is reserved, use from instead'));
    if(modelDefinition.properties._to) throw new Error(g.f('{{ArangoDB}} property _to is reserved, use to instead'));
    ...
    modelDefinition.properties.id = {
        name: 'id',
        type: String,
        arangodb: { column: '_id' }
    };

    modelDefinition.properties.rev = {
        name: 'rev',
        type: String,
...
ArangoDB.prototype.fromDatabaseItem = function(model, data) {
    ...
    /*for (let propName in modelObj.properties) {
        columnName = this.getDatabaseColumnName(model, propName);
        if(typeof data[columnName] !== 'undefined') result[propName] = data[columnName];
    }*/

    for (let columnName in data) {
        if(typeof data[columnName] !== 'undefined') {
            propName = this.getModelColumnName(model, columnName);
            if (propName) result[propName] = data[columnName];
            else result[columnName] = data[columnName];
        }
    }
...
ArangoDB.prototype.toDatabase = function(model, data) {
    ...
    if(data.constructor === Array) {
        for(let i in data) {
            data[i] = this.toDatabaseRow(model, data[i], defaultFn, settings);
        }

    } else {
        data = this.toDatabaseRow(model, data, defaultFn, settings);
    }

    return data;
};

ArangoDB.prototype.toDatabaseRow = function(model, data, defaultFn, settings) {
    if(!data) return null;
    ...
    if(settings.forceId === true || !data.key) {
        if(!settings.arangodb.unique) data.key = this.getDefaultFnID(defaultFn);
        else data.key = sha256(data.from + data.to);
    }
    ...
    for (let propName in data) {
        columnName = this.getDatabaseColumnName(model, propName);
        result[columnName] = data[propName] === null ? undefined : data[propName];
    }
...
ArangoDB.prototype.buildRecursiveFilter = function(model, prefix, where, preField, paramsCount) {
    ...
    preField = prefix + '.' + this.getDatabaseColumnName(model, preField);
    ...
    for(let i in where) {
        ...
        switch(i) {
            case 'and':
                sub = this.buildRecursiveFilter(model, prefix, where[i], i, paramsCount);
            ...
            default:
                if(typeof i === 'string' && typeof where[i] !== 'object') {
                    ...
                    let field = this.getDatabaseColumnName(model, i);
...
ArangoDB.prototype.buildIn = function(model, where) {
    ...
    //var nearFilter = self.findNearFilter(where);
    //customIn = 'WITHIN("' + self.collectionName(model) + '", @latitude, @longitude, @radius, @distanceAttributeName)';
...
ArangoDB.prototype.create = function(model, data, options, callback) {
    ...
    function createCallback(err, doc) {
        ...
        if(err) {
            console.log('Document Create ERROR: %j', err.response.body);
            return callback(err.response.body);
        }

        callback(null, { id: doc._id, key: doc._key } );
...
ArangoDB.prototype.update = ArangoDB.prototype.updateAll = function(model, where, data, options, callback) {
    ...
    self.buildQueryString(model, { where: where }, data, function(err, query) {

        self.db.query(query.string, (query.params || undefined), { count: true }, function(err, cursor) {

            if(options && options.returnValue === 'new') callback(null, self.fromDatabase(model, cursor._result));
            else callback(null, cursor.count);
