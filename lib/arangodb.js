...
ArangoDB.prototype.define = function(modelDefinition) {
    ...
    for(let i in modelDefinition.properties) {
        ...
        if(modelDefinition.properties[i].id) modelDefinition.ids.push(i);
    }
...
ArangoDB.prototype.parseTypeContent = function(model, propName, data) {
    ...
    if(columnType === Date) {
        if(data instanceof Date === false) data = new Date(data);
        result = data.getTime();
...
ArangoDB.prototype.getPropertyType = function(model, propName) {
    ...
    return this._models[model].properties[propName] && this._models[model].properties[propName].type || String;
...
ArangoDB.prototype.buildUpdate = function(model, data) {
    ...
    for(let i in data) {
        ...
        params['u' + paramsCount] = data[i] || this.getPropertyDefault(model, i) || null; //this.getPropertyType(model, i).prototype || null;
    }
...
ArangoDB.prototype.replaceById = function(model, id, data, options, callback) {
    ...
    //var type = self._models[model].settings.arangodb.type;

    if(id && id.indexOf('/') == -1) data.key = id;
    else data.id = id;
...
ArangoDB.prototype.updateOrCreate = function(model, data, options, callback) {
    ...
    var field;
    var where = {};
    for(var id in self._models[model].ids) {
        field = self._models[model].ids[id];
        if(!data[field]) return callback(new Error('All id fields are required (' + field + ')'));
...
ArangoDB.prototype.update = ArangoDB.prototype.updateAll = function(model, where, data, options, callback) {
    ...
    if(!where) return callback(new Error('No update allowed without WHERE params!'));
    ...
    if(!arangodbUtils.isEmpty(where)) {
        ...
        self.all(model, { where: where, fields: [primaryKey] }, function(err, items) {
            ...
            async.each(items, function(item, cb){
                self.collection(model).removeByKeys([item[primaryKey]], options, function(err, doc) {
                    if(err) {
